    ...
    new Command("set")
      ...
      .action(async (name: string, provider: string, model?: string) => {
        try {
          ...
          throw new CLIError(
            `Failed to set agent model: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      }),
  )
    ...
    new Command("get")
      ...
      .action(async (name: string) => {
        try {
          ...
          throw new CLIError(
            `Failed to get agent model: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      }),
  )
...
async function setAgentModel(
  ...
  if (availableModels.length > 0 && !availableModels.includes(model)) {
    logger.warn(
      `Model "${model}" not found in available models for ${provider}`,
    );
  ...
  logger.success(
    chalk.green(`✅ Updated agent "${agentName}" to use ${provider}:${model}`),
  );
}
  ...
  if (provider === "ollama") {
    try {
      const response = await fetch(
        `${config.llm.ollama?.baseUrl || "http://localhost:11434"}/api/tags`,
      );
      if (response.ok) {

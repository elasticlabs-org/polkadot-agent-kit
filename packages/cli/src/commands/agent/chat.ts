...
export const chatCommand = new Command('chat')
  ...
  .action(async (name: string, options: AgentChatOptions) => {
    try {
      ...
      throw new CLIError(`Failed to start chat session: ${error instanceof Error ? error.message : String(error)}`);
    }
  });

async function startChatSession(agentName: string, options: AgentChatOptions): Promise<void> {
  // Load agent
...
async function displayChatHistory(agentName: string): Promise<void> {
  ...
  try {
    ...
    history.slice(-10).forEach(message => {
      ...
      console.log(`${roleColor(`${roleIcon} ${message.role}`)} ${chalk.gray(`[${timestamp}]`)}`);
      console.log(chalk.white(message.content));
      console.log();
    });
  } catch (error) {
    logger.warn(`Failed to load chat history: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
async function runInteractiveChat(agent: AgentMetadata, options: AgentChatOptions): Promise<void> {
  ...
  console.log(chalk.green('🚀 Chat session started! Type your message and press Enter.'));
  console.log();
...
async function handleSpecialCommands(message: string, rl: readline.Interface, agent: AgentMetadata): Promise<boolean> {
...
async function saveChatMessage(agentName: string, role: 'user' | 'assistant', content: string): Promise<void> {
  ...
  if (await fs.pathExists(historyPath)) {
    try {
      ...
      logger.debug(`Failed to load existing history: ${error instanceof Error ? error.message : String(error)}`);
    }
  }

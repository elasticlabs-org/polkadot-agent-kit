...
export const deleteCommand = new Command('delete')
  ...
  .action(async (name: string, options: { yes?: boolean }) => {
    try {
      ...
      throw new CLIError(`Failed to delete agent: ${error instanceof Error ? error.message : String(error)}`);
    }
  });

async function deleteAgent(agentName: string, options: { yes?: boolean }): Promise<void> {
  ...
  try {
    ...
    throw new Error(`Failed to delete agent files: ${error instanceof Error ? error.message : String(error)}`);
  }
}

...
import { AgentListOptions, CLIError } from '../../types/commands.js';
...
export const listCommand = new Command('list')
  ...
  .action(async (options: AgentListOptions) => {
    try {
      ...
      throw new CLIError(`Failed to list agents: ${error instanceof Error ? error.message : String(error)}`);
    }
  });
...
async function loadAllAgents(): Promise<AgentMetadata[]> {
  ...
  for (const file of agentFiles) {
    try {
      ...
      logger.warn(`Failed to load agent from ${file}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
...
function displayAgentsTable(agents: AgentMetadata[]): void {
  ...
  agents.forEach(agent => {
    const toolsStr = agent.tools.length > 3 

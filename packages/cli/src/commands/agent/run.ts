...
import type { AgentRunOptions } from "../../types/commands";
...
runCommand
  ...
  .action(async (options: { agent: string; chain?: string }, cmd) => {
    ...
    try {
      await runCheckBalanceCommand(
        agentName,
        options.chain || "polkadot",
        parentOptions,
      );
    } catch (error) {
      throw new CLIError(
...
runCommand
  ...
  .action(
    async (
      amount: string,
      address: string,
      options: { agent: string; chain?: string },
      cmd,
    ) => {
      const parentOptions = cmd.parent?.opts() as AgentRunOptions;
      const agentName = options.agent;
  ...
  .action(
    async (
      ...
      try {
        ...
        throw new CLIError(
          `Failed to execute XCM transfer: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    },
  );
...
async function runCheckBalanceCommand(
  agentName: string,
  chain: string,
  ...
  logger.info(
    chalk.blue(`🚀 Checking balance for agent "${agentName}" on ${chain}...`),
  );
...
async function runTransferCommand(
  ...
  logger.info(
    chalk.blue(
      `🚀 Transferring ${amount} tokens to ${address} on ${chain} with agent "${agentName}"...`,
    ),
  );
...
async function runXcmCommand(
  ...
  logger.info(
    chalk.blue(
      `🚀 XCM transfer of ${amount} tokens to ${address} from ${sourceChain} to ${destChain} with agent "${agentName}"...`,
    ),
  );

  try {
    const result = await executeXcmTransferCommand(
      agent,
      amount,
      address,
      sourceChain,
    ...
    logger.error(
      `XCM transfer failed: ${error instanceof Error ? error.message : String(error)}`,
    );
    process.exit(1);
  }
}
...
async function runSwapCommand(
  ...
  logger.info(
    chalk.blue(
      `🚀 Swapping ${amount} ${fromCurrency} to ${toCurrency} with agent "${agentName}"...`,
    ),
  );

  try {
    const result = await executeSwapCommand(
      agent,
      amount,
      fromCurrency,
      toCurrency,
...
function displayAgentInfo(agent: AgentMetadata): void {
  ...
  console.log(
    chalk.gray(
      `Networks: ${agent.polkadotConfig.chains ? agent.polkadotConfig.chains.join(", ") : "All supported networks"}`,
    ),
  );
  console.log();
}
...
async function executeBalanceCheckCommand(
  agent: AgentMetadata,
  chain: string,
): Promise<any> {
  try {
...
async function executeTransferCommand(
  agent: AgentMetadata,
  amount: string,
  address: string,
  chain: string,
  ...
  try {
    ...
    const transferResult = await transferTool.call({
      to: address,
      amount,
      chain,
    });
...
async function executeXcmTransferCommand(
  agent: AgentMetadata,
  amount: string,
  address: string,
  sourceChain: string,
  ...
  try {
    ...
    logger.info(
      `XCM transfer of ${amount} tokens to ${address} from ${sourceChain} to ${destChain}`,
    );

    // Get the XCM transfer tool and execute it
    const xcmTool = agentKit.xcmTransferNativeTool();
    const xcmResult = await xcmTool.call({
      to: address,
      amount,
      sourceChain,
      destChain,
...
async function executeSwapCommand(
  agent: AgentMetadata,
  amount: string,
  fromCurrency: string,
  toCurrency: string,
...
function displayRawResult(result: any): void {
  ...
  if (result.type === "balance") {
    ...
    console.log(
      chalk.gray(
        `From: ${result.data.sourceChain} to ${result.data.destChain}`,
      ),
    );
    console.log(chalk.white(JSON.stringify(result.data.result, null, 2)));
    ...
    console.log(
      chalk.gray(
        `From: ${result.data.fromCurrency} to ${result.data.toCurrency}`,
      ),
    );
    if (result.data.swapOptions.fromChain && result.data.swapOptions.toChain) {
      console.log(
        chalk.gray(
          `Chains: ${result.data.swapOptions.fromChain} to ${result.data.swapOptions.toChain}`,
        ),
      );
    }

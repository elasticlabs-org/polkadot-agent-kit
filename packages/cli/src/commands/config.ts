      ...
      .action(
        async (
          ...
          try {
            ...
            logger.success(
              `Set ${key} = ${JSON.stringify(parsedValue)} in ${scope} scope`,
            );
          } catch (error) {
            throw new CLIError(
    ...
    new Command("edit")
      ...
      .action(async (options: { global?: boolean; project?: boolean }) => {
        try {
          ...
          throw new CLIError(
            `Failed to edit configuration: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      }),
  )
  ...
  .addCommand(
    new Command("paths")
      ...
      .action(() => {
        ...
        logger.info(
          `  Global: ${chalk.cyan(configManager.getGlobalConfigPath())}`,
        );
        logger.info(
          `  Project: ${chalk.cyan(configManager.getProjectConfigPath())}`,
        );
      }),
  );

function displayConfigTable(config: Record<string, any>): void {
  const data = [[chalk.bold("Key"), chalk.bold("Value"), chalk.bold("Type")]];
...
async function editAgentSettings(scope: "global" | "project"): Promise<void> {
  ...
  await configManager.set(
    "agents.storageLocation",
    answers.storageLocation,
    scope,
  );
...
async function editPolkadotSettings(
  scope: "global" | "project",
): Promise<void> {
  const config = configManager.getConfig();

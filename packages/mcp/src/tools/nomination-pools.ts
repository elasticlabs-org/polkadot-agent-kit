...
export class NominationPoolsToolHandler {
  ...
  async joinPool(request: CallToolRequest): Promise<CallToolResult> {
    return executeTool<StakingResult>(
      "join_pool",
      async () => {
        const params = joinPoolSchema.parse(request.params) as JoinPoolParams;
      ...
      (result) => {
        if (result.success) {
          return `Successfully joined nomination pool with ${request.params.amount} tokens on ${request.params.chain}. Transaction hash: ${result.transactionHash}`;
        } else {
  ...
  async bondExtra(request: CallToolRequest): Promise<CallToolResult> {
    return executeTool<StakingResult>(
      "bond_extra",
      async () => {
        const params = bondExtraSchema.parse(request.params) as BondExtraParams;
      ...
      (result) => {
        if (result.success) {
          return `Successfully bonded extra tokens to nomination pool on ${request.params.chain}. Transaction hash: ${result.transactionHash}`;
        } else {
  ...
  async unbond(request: CallToolRequest): Promise<CallToolResult> {
    return executeTool<StakingResult>(
      "unbond",
      async () => {
        const params = unbondSchema.parse(request.params) as UnbondParams;
        const { amount, chain } = params;
      ...
      (result) => {
        if (result.success) {
          return `Successfully unbonded ${request.params.amount} tokens from nomination pool on ${request.params.chain}. Transaction hash: ${result.transactionHash}`;
        } else {
  ...
  async withdrawUnbonded(request: CallToolRequest): Promise<CallToolResult> {
    return executeTool<StakingResult>(
      "withdraw_unbonded",
      async () => {
        const params = withdrawUnbondedSchema.parse(request.params) as WithdrawUnbondedParams;
      ...
      (result) => {
        if (result.success) {
          return `Successfully withdrew unbonded tokens from nomination pool on ${request.params.chain}. Transaction hash: ${result.transactionHash}`;
        } else {
  ...
  async claimRewards(request: CallToolRequest): Promise<CallToolResult> {
    return executeTool<StakingResult>(
      "claim_rewards",
      async () => {
        const params = claimRewardsSchema.parse(request.params) as ClaimRewardsParams;
        const { chain } = params;
      ...
      (result) => {
        if (result.success) {
          return `Successfully claimed rewards from nomination pool on ${request.params.chain}. Transaction hash: ${result.transactionHash}`;
        } else {

...
import { type CheckBalanceParams, checkBalanceSchema } from "../types/schemas";
...
export class BalanceToolHandler {
  ...
  async checkBalance(request: CallToolRequest): Promise<CallToolResult> {
    return executeTool<BalanceResult>(
      "check_balance",
      async () => {
        const params = checkBalanceSchema.parse(request.params);
        const { chain } = params;

        const targetAddress = this.polkadotAgent.getCurrentAddress();
        if (!targetAddress) {
          throw new Error(
            "No address provided and no default address configured",
          );
        }
      ...
      (result) =>
        `Balance on ${result.chain}: ${result.balance} ${result.symbol}`,
    );
  }
}

...
import {
  BalanceToolHandler,
  NominationPoolsToolHandler,
  SwapToolHandler,
  TransferToolHandler,
  XcmTransferToolHandler,
} from "./tools";
...
export class PolkadotMCPServer {
  ...
  constructor(
    config: PolkadotMCPServerConfig,
    polkadotAgent: PolkadotAgentKit,
  ) {
    ...
    this.nominationPoolsToolHandler = new NominationPoolsToolHandler(
      this.polkadotAgent,
    );
    this.xcmTransferToolHandler = new XcmTransferToolHandler(
      this.polkadotAgent,
    );
  ...
  private registerTools(): void {
    ...
    this.server.tool(
      ...
      "Check native token balance for the current account on a specified blockchain network",
      {
        chain: z
          ...
          .describe("The blockchain network to check balance on"),
      },
      async ({ chain }) => {
        try {
          const request = {
    ...
    this.server.tool(
      ...
      {
        to: z.string().describe("Recipient's address in SS58 format"),
        amount: z
          .string()
          .regex(/^\d+(\.\d+)?$/, "Amount must be a valid number")
          .describe(
            "Amount to transfer in the native token (e.g., '1.5' for 1.5 DOT)",
          ),
        chain: z
          .enum(["polkadot", "kusama", "westend"])
          .describe("The blockchain network to perform the transfer on"),
      },
    ...
    this.server.tool(
      ...
      "Join a nomination pool for staking tokens on a specified blockchain network",
      {
        amount: z
          ...
          .regex(/^\d+(\.\d+)?$/, "Amount must be a valid number")
          .describe(
            "Amount of tokens to join the pool with (e.g., '10' for 10 DOT)",
          ),
        chain: z
          .enum(["polkadot", "kusama", "westend"])
          .describe("The blockchain network to join the pool on"),
      },
      async ({ amount, chain }) => {
        try {
          const request = {
    ...
    this.server.tool(
      ...
      "Bond extra tokens to an existing nomination pool",
      {
        type: z
          ...
          .describe(
            "Type of bonding: 'FreeBalance' to bond from wallet, 'Rewards' to restake earned rewards",
          ),
        amount: z
          .string()
          .regex(/^\d+(\.\d+)?$/, "Amount must be a valid number")
        ...
        chain: z
          .enum(["polkadot", "kusama", "westend"])
          .describe("The blockchain network to bond on"),
      },
      async ({ type, amount, chain }) => {
        try {
          ...
          return await this.nominationPoolsToolHandler.bondExtra(
            request as any,
          );
        } catch (error) {
          const errorMessage =
    ...
    this.server.tool(
      ...
      "Unbond tokens from a nomination pool",
      {
        ...
        chain: z
          .enum(["polkadot", "kusama", "westend"])
          .describe("The blockchain network to unbond from"),
      },
      async ({ amount, chain }) => {
        try {
          const request = {
    ...
    this.server.tool(
      ...
      "Withdraw unbonded tokens from a nomination pool",
      {
        slashingSpans: z
          .string()
          .describe("The number of slashing spans (usually '0')"),
        chain: z
          .enum(["polkadot", "kusama", "westend"])
          .describe("The blockchain network to withdraw from"),
      },
      async ({ slashingSpans, chain }) => {
        try {
          ...
          return await this.nominationPoolsToolHandler.withdrawUnbonded(
            request as any,
          );
        } catch (error) {
          const errorMessage =
    ...
    this.server.tool(
      ...
      "Claim rewards from a nomination pool",
      {
        chain: z
          .enum(["polkadot", "kusama", "westend"])
          .describe("The blockchain network to claim rewards from"),
      },
      async ({ chain }) => {
        try {
          const request = {
          ...
          return await this.nominationPoolsToolHandler.claimRewards(
            request as any,
          );
        } catch (error) {
          const errorMessage =
    ...
    this.server.tool(
      ...
      "Transfer native tokens across chains using XCM (Cross-Consensus Messaging)",
      {
        ...
        to: z.string().describe("Recipient address in SS58 format"),
        sourceChain: z
          .enum([
            "polkadot",
            "kusama",
    ...
    this.server.tool(
      ...
      "Swap tokens across chains or within specific DEXs",
      {
        from: z
          ...
          .optional()
          .describe(
            "Source chain for cross-chain swap (e.g., 'Polkadot', 'Hydra')",
          ),
        to: z
          .string()
          .optional()
        ...
        currencyTo: z
          .string()
          .describe(
            "Token symbol to swap to (e.g., 'DOT', 'KSM', 'HDX', 'USDT')",
          ),
        amount: z
          .string()
          .regex(/^\d+(\.\d+)?$/, "Amount must be a valid number")

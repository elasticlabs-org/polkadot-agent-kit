...
describe('PolkadotAgentKit Integration with OllamaAgent for XCM Transfer', () => {
  ...
  describe('Parachain to Parachain Transfers', () => {
    const parachainToParachainCases = [
      {
        name: 'Paseo Asset Hub to Paseo People 1 ',
        query: `transfer 0.1 PAS to ${RECIPIENT2} from Paseo Asset Hub to Paseo People`,
        recipient: RECIPIENT2,
        sourceChainId: 'paseo_asset_hub',
        destChainId: 'paseo_people',
        expectedSourceChain: 'AssetHubPaseo',
        expectedDestChain: 'PeoplePaseo'
      },
      {
        name: 'Paseo Asset Hub to Paseo People 2',
        query: `transfer 0.1 PAS to ${RECIPIENT3} from Asset Hub Paseo to People Paseo`,
    ...
    parachainToParachainCases.forEach(({ name, query, recipient, sourceChainId, destChainId, expectedSourceChain, expectedDestChain }) => {
